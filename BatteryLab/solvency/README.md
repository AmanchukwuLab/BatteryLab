...
# Solution setup
For some specified final composition $`\vec{v}_{\text{final}}`$ and $`\vec{m}_{\text{final}}`$, the goal is to find the set of volume fractions $`\vec{v}_{\text{components}}`$ of some minimal set of the electrolytes available that yield the final composition. The available electrolyte solutions are represented as a volume fraction matrix, $`\textbf{S}`$, and a molarity matrix, $`\textbf{M}`$, such that
```math
\textbf{S}\vec{v}_{\text{components}}=\vec{v}_{\text{final}},
``` 
and 
```math
\textbf{M}\vec{v}_{\text{components}}=\vec{m}_{\text{final}}
```
...
$$\textbf{S} = \begin{bmatrix} 1.0 & 1.0 \\ 0.0 & 0.0 \end{bmatrix}$$
$$\textbf{M} = \begin{bmatrix} 0.0 & 0.0 \\ 0.0 & 5.0 \end{bmatrix}$$
-->
